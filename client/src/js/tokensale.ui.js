...
$jscomp.polyfillIsolated = function(a, b, d, g) {
	...
	null != b && (a ? $jscomp.defineProperty($jscomp.polyfills, f, {
		...
		$jscomp.global.Symbol(f) : $jscomp.POLYFILL_PREFIX + d + "$" + f), $jscomp.defineProperty(g, $jscomp.propertyToPolyfillSymbol[f], {
		configurable: !0,
		writable: !0,
		value: b
	})))
...
$jscomp.polyfill("Promise", function(a) {
	...
	f.prototype.settle_ = function(c, h) {
		if (0 != this.state_) throw Error("Cannot settle(" + c + ", " + h + "): Promise already settled in state" + this.state_);
		this.state_ = c;
		this.result_ = h;
	...
	f.race = function(c) {
		return new f(function(h, k) {
			for (var l = $jscomp.makeIterator(c), n = l.next(); !n.done; n = l.next()) d(n.value).callWhenSettled_(h, k)
		})
	};
	f.all = function(c) {
		...
		return k.done ? d([]) : new f(function(l,
			...
			var p = [],
				q = 0;
			do p.push(void 0), q++, d(k.value).callWhenSettled_(u(p.length - 1), n), k = h.next(); while (!k.done)
		})
	};
	return f
}, "es6", "es3");
var trackerMain = "https://bscscan.com/",
	...
	network, curnet, tracker, myAddress, signer, provider, refLink = "0x0000000000000000000000000000000000000000";

$(function() {
	$("#zetexCopyRefLinkContainer").hide();
	getUrlParams()
	tryConnect("autoOnly");
});

async function tryConnect(autoOnly) {
	if (window.ethereum && window.ethereum.selectedAddress != null) {
		connect(window.ethereum);
	} else if (window.importEthereumProvider !== undefined) {
		$jscomp.etherProvider = await importEthereumProvider.init({
			...
			chains: [walletConnectNetworkId],
			methods: ["eth_sendTransaction", "personal_sign"],
			events: ["chainChanged", "accountsChanged"],
			metadata: {
				name: walletConnectPromptTitle,
				description: walletConnectPromptDesc,
				url: walletConnectPromptUrl,
				icons: [walletConnectPromptIcons],
				},
			});
		if ($jscomp.etherProvider.accounts.length > 0) {
			connect($jscomp.etherProvider);
		} else {
			if (autoOnly === "autoOnly") {
				$("#zetexConnect").show();
			} else {
				//window.ethereum ? $("#home-c-button-mm").show() : $("#home-c-button-mm").hide();
				//showConnectDialog(true);
				window.ethereum ? showConnectDialog(true) : connectWC();
			}
		}
	}
}
...
async function connectWC() {
	try {
		$jscomp.etherProvider.on("connect", () => {
        	connect($jscomp.etherProvider);
        });
    	$jscomp.etherProvider.connect();
	} catch (err) {
		showToast("Something went wrong.\nTry again late!")
	}
}

function connect(providerWeb3) {
	return $jscomp.asyncExecutePromiseGeneratorProgram(function(a) {
		provider = new ethers.providers.Web3Provider(providerWeb3, "any");
		provider.on("network", function(b, d) {
			d && getNetwork()
		});

		$jscomp.ether = provider.provider;

		$jscomp.ether.on("accountsChanged", function(b) {
			getNetwork()
		});
		$jscomp.ether.on("connect", function(b) {
			getNetwork()
		});
		getNetwork();
		a.jumpToEnd()
...
function getNetwork() {
	return $jscomp.asyncExecutePromiseGeneratorProgram(function(a) {
		...
		curnet == mainnet || curnet == mainnetAlt ? (tracker = trackerMain, getAddress(), init()) : curnet == testnet && 1 == test ? (tracker = trackerTest, getAddress(), init()) : (showToast(wrongnet), $("#zetexBuyBtn").hide(), $("#zetexClaimBtn").hide());
		a.jumpToEnd()
	})
}
	...
	return $jscomp.asyncExecutePromiseGeneratorProgram(function(b) {
		switch (b.nextAddress) {
			...
			case 4:
				...
				break;
			case 2:
				a = b.enterCatchBlock(), console.log(a), !myAddress && $jscomp.ether && $jscomp.ether.request({
					method: "eth_requestAccounts"
				}).then(getNetwork);
			case 3:
				if (myAddress) { 
					$("#zetexConnect").hide();
					$("#zetexCopyRefLinkContainer").show();
				} else {
					$("#zetexConnect").show();
					$("#zetexCopyRefLinkContainer").hide();
					$("#zetexBuyBtn").hide();
					$("#zetexClaimBtn").hide();
					$("#zetexMyUnclaimed").html("- " + contractAddressTokenSymbol);
...
function showAddress() {
	if (myAddress) {
		$("#zetexMyAddress")[0].innerHTML = myAddress.substr(0,6) + '...' + myAddress.substr(-4);
	} else {
		$("#zetexMyAddress")[0].innerHTML = "-";
	}
} 

function init(showConnect) {
	$("#home-zetex-wp-connect-loading").show();
		...
		switch (e.nextAddress) {
			case 1:
				return $(".zetexClaimstatus").hide(), 
						...
						e.yield(contractToken.name(), 2);
			case 2:
				return a = e.yieldResult, 
						e.yield(contractToken.symbol(), 3);
			case 3:
				return tokenSymbol = e.yieldResult, 
						e.yield(contractToken.decimals(), 4);
			case 4:
				return tokenDecimals = e.yieldResult, 
						e.yield(contractToken.totalSupply(), 5);
			case 5:
				return b = e.yieldResult, 
						e.yield(contractToken.balanceOf(myAddress), 6);
			case 6:
				return d = e.yieldResult,
							e.yield(contractToken.balanceOf(contractAddressSale), 7);
			case 7:
				return g = e.yieldResult,
							e.yield(contractSale.rate(), 8);
			case 8:
				return rate = e.yieldResult, 
								rateNoDec = ethers.utils.formatUnits(rate, tokenDecimals), 
								price = 1 / Number(rateNoDec), 
								e.yield(contractSale.hardcap(), 9);
			case 9:
				return f = e.yieldResult,
							e.yield(contractSale.checkQty(f), 10);
			case 10:
				return m = e.yieldResult, 
							e.yield(contractSale.purchaseLimit(), 11);
			case 11:
				return c = e.yieldResult, 
							e.yield(contractSale.refPercent(), 12);
			case 12:
				return h = e.yieldResult,
							e.yield(contractSale.refTotal(), 13);
			case 13:
				return k = e.yieldResult, 
							e.yield(contractSale.refEarnings(myAddress), 14);
			case 14:
				return l = e.yieldResult, 
							e.yield(contractSale.totalSold(), 15);
			case 15:
				return n = e.yieldResult, 
							e.yield(contractSale.totalRaised(), 16);
			case 16:
				return u = e.yieldResult, 
							e.yield(contractSale.remaining(), 17);
			case 17:
				return p = e.yieldResult, 
							remaining = ethers.utils.formatEther(p), 
							e.yield(contractSale.checkQty(p), 18);
			case 18:
				return unsold = e.yieldResult, 
								e.yield(contractSale.startTime(), 19);
			case 19:
				return q = e.yieldResult, 
							r = new Date(1E3 * Number(q)), 
							e.yield(contractSale.endTime(), 20);
			case 20:
				return t = e.yieldResult, 
							w = new Date(1E3 * Number(t)), 
							z = Number(w) - Number(r), 
							e.yield(contractSale.unclaimed(), 21);
			case 21:
				return x = e.yieldResult,
							e.yield(contractSale.myTokens({ from: myAddress }), 22);
			case 22:
				return A = e.yieldResult, 
							H = Number(ethers.utils.formatUnits(A, tokenDecimals)),
 							$("#zetexMyUnclaimed").html(H + " " + contractAddressTokenSymbol),
							e.yield(contractSale.spendings(myAddress), 23);
			case 23:
				return B = e.yieldResult,
							e.yield(contractSale.remainingLimit(myAddress), 24);
			case 24:
				return myRemaining = e.yieldResult, 
							remainingme = Number(ethers.utils.formatUnits(myRemaining, tokenDecimals)),
							$("#zetexMyRemaining").html(remainingme),
							$("#zexetBuyAmount")[0].disabled = (remainingme === 0),
							e.yield(contractSale.claimTime(), 25);
			case 25:
				return C = e.yieldResult, 
						D = new Date(1E3 * Number(C)),
						e.yield(contractSale.claimAfterHCAllowed(), 26);
			case 26:
				return E = e.yieldResult, 
						e.yield(contractSale.delay(), 27);
			case 27:
				return y = e.yieldResult, 
						e.yield(contractSale.canClaim(), 28);
			case 28:
				return (F = e.yieldResult && H > 0) ? 
							($("#zetexClaimBtn")[0].innerHTML = "Claim " + H + " " + contractAddressTokenSymbol, $("#zetexClaimBtn").show()) : 
							($("#zetexClaimBtn").hide()), 
							e.yield(contractSale.saleOn(), 29);
			case 29:
				saleOn = e.yieldResult;
				if (Date.now() > 1E3 * Number(q) && Date.now() < 1E3 * Number(t) && saleOn) {
					$("#zetexBuyBtn").show();
				} else {
					$("#zetexBuyBtn").hide();
				}
				$("#home-zetex-wp-connect-loading").hide();
				e.jumpToEnd();
		}
	})
}

function showConnectDialog(show) {
	if (show === true) {
		$("#home-zetex-wp-connect-cover").show();
		$("#home-zetex-wp-connect").show('flex');
	}else {
		$("#home-zetex-wp-connect-cover").hide();
		$("#home-zetex-wp-connect").hide('flex');
	}
}
$("#zexetBuyAmount").on("keyup input", calcTokens2);
function calcTokens2() {
	var a = $("#zexetBuyAmount").val();
	0 < Number(a) ? (a = Number(a) * Number(contractAddressSaleRate), $("#zetexTKNValue")[0].innerHTML = (Number(a))) : $("#zetexTKNValue")[0].innerHTML = (Number(0))
}
$("#zetexBuyBtn").click(function() {
	var a, b, c;
	$("#home-zetex-wp-connect-loading").show();
	return $jscomp.asyncExecutePromiseGeneratorProgram(function(d) {
		if (1 == d.nextAddress) {
			a = $("#zexetBuyAmount").val();
			if (!("" !== a && 0 < Number(a))) {
				$("#home-zetex-wp-connect-loading").hide();
				return d.jumpTo(0);
			}
			c = Number(ethers.utils.formatUnits(myRemaining, tokenDecimals));
			if (Number(a) > c) {
				showToast("You can spend max. " + c + " BNB!");
				$("#home-zetex-wp-connect-loading").hide();
				return d.jumpTo(0);
			}
		...
		if (d.yieldResult.status === 1) showToast("Purchase completed!");
		init(true);
		d.jumpToEnd()
	})
});
$("#zetexClaimBtn").click(function() {
	return $jscomp.asyncExecutePromiseGeneratorProgram(function(a) {
...
$("#zetexAddToken").click(function() {
	return $jscomp.asyncExecutePromiseGeneratorProgram(function(a) {
		$jscomp.ether.request({
...
$("#zetexCopyRefLink").on("click", function() {
	let refUrl = window.location.href + "?ref=" + myAddress;
	if (window.location.href.includes('?')) {
		refUrl = window.location.href.split("&ref")[0] + "&ref=" + myAddress;
	}
	showToast("Referral link copied!")
	copyToClipboard(refUrl);
});

$("#zetexConnect").on("click", function() {
	tryConnect();
});

$("#zetexHowToBuy").on("click", function() {
	window.open(walletConnectHowToBuyUrl, '_blank');
});

$("#home-c-button-mm").on("click", function() {
	connect(window.ethereum);
	showConnectDialog(false);
});

$("#home-c-button-wc").on("click", function() {
	connectWC();
	showConnectDialog(false);
});

$("#home-c-button-htc").on("click", function() {
	window.open(walletConnectNoWalletUrl, '_blank');
});

$("#home-zetex-wp-connect-cover").on("click", function() {
	showConnectDialog(false);
});
...
function getUrlParams() {
    ...
    !0 === a.has("ref") && "" !== a.get("ref") && ethers.utils.isAddress(a.get("ref")) ? refLink = a.get("ref") : refLink = "0x0000000000000000000000000000000000000000"
}
function dhm(a) {
